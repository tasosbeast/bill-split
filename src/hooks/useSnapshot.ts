...
} from "../types/legacySnapshot"; // TODO Phase 5: rename legacySnapshot.ts → snapshot.ts
import { useAppStore } from "../state/appStore";
export type SetStateAction<T> = T | ((prev: T) => T);

export interface SnapshotUpdaters {
...
export function useSnapshot(): UseSnapshotResult {
  ...
  const updaters = useMemo<SnapshotUpdaters>(
    () => ({
      setFriends: setFriends as SnapshotUpdaters["setFriends"],
      setSelectedId: setSelectedId as SnapshotUpdaters["setSelectedId"],
      setTransactions: setTransactions as SnapshotUpdaters["setTransactions"],
      setTemplates: setTemplates as SnapshotUpdaters["setTemplates"],
      replaceSnapshot,
      reset,
    }),
    [
