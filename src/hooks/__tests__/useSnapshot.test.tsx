...
import type { StoredTransaction } from "../../types/legacySnapshot"; // TODO Phase 5 rename
...
describe("useSnapshot", () => {
  ...
  it("seeds default friends when no snapshot is stored", () => {
    ...
    expect(storageMock.saveState).toHaveBeenCalledWith(result.current.snapshot);
  ...
  it("drops selectedId when the selected friend is removed", () => {
    ...
    const { result, rerender, unmount } = renderHook(() => useSnapshot());
  ...
  it("applies upgradeTransactions whenever transactions change", () => {
    ...
    expect(upgradeTransactionsMock).toHaveBeenCalledWith([{ id: "tx-1" }]);
